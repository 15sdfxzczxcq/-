...
public:
  ...
  //上位机控制pd发送数据
  /*void carControl(float speed, uint16_t servo){
  int speedpwm = abs(speed);
      
      transmitByte('A');
      if(speed>=0)transmitByte(0);
      if(speed<0)transmitByte(1);
      transmitByte(speedpwm/10);
      transmitByte(speedpwm%10);
      transmitByte((servo-1000)/100);
      transmitByte(((servo-1000)%100)/10);
      transmitByte(0);
      transmitByte(0);
 ...
 /*void carControl(float speed, uint16_t servo){
  int speedpwm = abs(speed);
      ...
      transmitByte(speedpwm/10);
      transmitByte(speedpwm%10);
      transmitByte(servo/100);
      transmitByte(servo/10%10);
      transmitByte(servo%10);
      transmitByte(0);
     // transmitByte(buzzerstate);
      transmitByte(0);
      transmitByte('B');
...
void carControl(float speed, uint16_t servo , bool buzzer, float state) {
    if (!isOpen)
        return;
    ...
    data[6] = servo % 10;
    data[7] = buzzer;
    data[8] = speedpwm +(servo / 100)+((servo % 100) / 10)+(servo % 10);
    data[9] = 'B';
 
    // 使用 DataBuffer 格式发送
    try {
        serialPort->Write(data); // 直接传入 vector
        serialPort->DrainWriteBuffer();
    } catch (const std::exception &e) {
        std::cerr << "Write failed: " << e.what() << std::endl;
    }

    //std::cout << "Speed: " << speedpwm << " Servo: " << servo << std::endl;
}
