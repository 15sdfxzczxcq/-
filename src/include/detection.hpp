 ...
 * @file detection.hpp
 * @author Leo
 * @brief 基于Paddle飞桨，Edgeboard板卡部署的AI目标检测框架（支持按类型设置置信度阈值）
 * @version 0.2
 * @date 2024-02-20
 *
...
public:
    ...
    float score = 0.5;                  // 全局检测置信度阈值
    std::unordered_map<int, float> type_score_thresholds; // 各类别专用置信度阈值
    ...
    Detection(const std::string pathModel)
    {
        ...
        setTypeScoreThreshold(LABEL_PEDESTRIAN, 0.3f);
        // 设置默认的障碍物阈值（可根据需要修改）
        setTypeScoreThreshold(LABEL_BLOCK, 0.3f);
        setTypeScoreThreshold(LABEL_CAR, 0.1f);
        setTypeScoreThreshold(LABEL_BURGER, 0.6f);
    };

    /**
     * @brief 设置特定类型的置信度阈值
     * 
     * @param type 物体类型ID (如LABEL_BLOCK)
     * @param threshold 置信度阈值(0.0-1.0)
     */
    void setTypeScoreThreshold(int type, float threshold) {
        type_score_thresholds[type] = std::clamp(threshold, 0.0f, 1.0f);
    }
     ...
     * 
     * @param type 物体类型ID
     * @return float 该类型的置信度阈值，如果未设置则返回全局阈值
     */
    float getTypeScoreThreshold(int type) const {
        auto it = type_score_thresholds.find(type);
        return (it != type_score_thresholds.end()) ? it->second : score;
    }
    ...
    {
        auto feeds = preprocess(img, {320, 320});
        run(*feeds);
        render();
    }
        ...
        const std::vector<int64_t> &input_size)
    {
        ...
        cv::resize(x, x, cv::Size(input_size[0], input_size[1]), 0, 0, cv::INTER_LINEAR);
    ...
    void render()
    {
        ...
        for (int i = 0; i < prod; i += 6)
        {
            ...
            // 使用类型专用阈值进行过滤
            float type_threshold = getTypeScoreThreshold(result.type);
            if (result.score < type_threshold) {
                continue;
            }
    ...
    void drawBox(cv::Mat &img)
    {
        for (size_t i = 0; i < results.size(); i++)
        {
            ...
            cv::rectangle(img, rect, getCvcolor(result.type), 2);
    ...
    {
        // 为障碍物使用特殊颜色
        if(index == LABEL_BLOCK) {
            return cv::Scalar(255, 255, 255); // 白色边框
        }
        
        switch (index)
        {
        case 0: return cv::Scalar(0, 255, 0);   // 绿
        case 1: return cv::Scalar(255, 255, 0); // 天空蓝
        case 2: return cv::Scalar(0, 0, 255);   // 大红
        case 3: return cv::Scalar(0, 250, 250); // 大黄
        case 4: return cv::Scalar(250, 0, 250); // 粉色
        case 5: return cv::Scalar(0, 102, 255); // 橙黄
        case 6: return cv::Scalar(255, 0, 0);   // 深蓝
        case 7: return cv::Scalar(255, 255, 255); // 大白
        case 8: return cv::Scalar(247, 43, 113);
        case 9: return cv::Scalar(40, 241, 245);
        case 10: return cv::Scalar(237, 226, 19);
        case 11: return cv::Scalar(245, 117, 233);
        case 12: return cv::Scalar(55, 13, 19);
        case 13: return cv::Scalar(255, 255, 255);
        case 14: return cv::Scalar(237, 226, 19);
        case 15: return cv::Scalar(0, 255, 0);
