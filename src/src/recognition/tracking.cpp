...
public:
  ...
  int left_count = 0;               // 边缘正常行数（左）
  int right_count = 0;              // 边缘正常行数（右）
  ...
  void trackRecognition(bool isResearch, uint16_t rowStart) {
    ...
    for (int row = rowStart; row > rowCutUp; row--) // 有效行：10~220
    {
      ...
      {
        if (imagePath.at<uchar>(row, 1)  == 255) {
          startBlock[counterBlock] = 0;
        }
        for (int col = 1; col < COLSIMAGE; col++) // 搜索出每行的所有色块
        {
          if (imagePath.at<uchar>(row, col) == 255 &&
              imagePath.at<uchar>(row, col - 1) < 255) {
            startBlock[counterBlock] = col;
          } else {
            if (imagePath.at<uchar>(row, col) < 255 &&
                imagePath.at<uchar>(row, col - 1) == 255) {
        ...
        //if(counterBlock == 0){
        if (imagePath.at<uchar>(row, COLSIMAGE - 1) == 255) {
          if (counterBlock < end(endBlock) - begin(endBlock) - 1)
            endBlock[counterBlock++] = COLSIMAGE - 1;
        }
      //}
      ...
      if (flagStartBlock)                            // 起始行做特殊处理
      {
        ...
        int limitWidthBlock = COLSIMAGE * 0.3; // 首行色块宽度限制（不能太小）
      ...
      } else // 其它行色块坐标处理
      {
        ...
        //2025.7.6新加
          if(pointsEdgeLeft.size() > 0&&pointsEdgeRight.size() > 0){
          if (abs(startBlock[indexBlocks[0]] -startpointy)>3 && pointsEdgeLeft[pointsEdgeLeft.size() - 1].y < 5)
          {
              startpointy = startBlock[indexBlocks[0]];
              startBlock[indexBlocks[0]] = 0;
              //std::cout<<"左边出现"<<std::endl; 
   
          }

          if (abs(endBlock[indexBlocks[0]] - endpointy)>3 && pointsEdgeRight[pointsEdgeRight.size() - 1].y > (COLSIMAGE-5))
          {
              endpointy = endBlock[indexBlocks[0]];
              endBlock[indexBlocks[0]] = COLSIMAGE;
              //std::cout<<"右边出现"<<std::endl; 
          }
        }
          //2025.7.6结束
                   ...
                   1) // 存在多个色块，则需要择优处理：选取与上一行最近的色块
        {
          ...
          //2025.7.6新加
          if(pointsEdgeLeft.size() > 0&&pointsEdgeRight.size() > 0){
          if (abs(startBlockNear - startpointy)>3 && pointsEdgeLeft[pointsEdgeLeft.size() - 1].y < 5)
          {
              startpointy = startBlockNear;
              startBlockNear = 0;
              //std::cout<<"左边出现"<<std::endl; 
              //std::cout<<pointsEdgeLeft[pointsEdgeLeft.size() - 1].x<<std::endl; 
              //  for (int i = 0; i < COLSIMAGE; i++)
              // {
              //   circle(imagePath, Point(i,pointsEdgeLeft[pointsEdgeLeft.size() - 2].x), 2,
              //           Scalar(255, 255, 255), -1); // 粉色
              // }
          }

          if (abs(endBlockNear - endpointy)>3 && pointsEdgeRight[pointsEdgeRight.size() - 1].y > (COLSIMAGE-5))
          {
              endpointy = endBlockNear;
              endBlockNear = COLSIMAGE;
              //std::cout<<"右边出现"<<std::endl; 
          }
        }
          //2025.7.6结束
 ...
 double stdevEdgeCal(vector<POINT> &v_edge, int img_height) {
  ...
  size_t end_index = 5 * v_edge.size() / 6;
  
  vector<int> v_slope;
  int step = 10;
  
  // 确保从 start_index + step 开始计算（避免下标越界）
  for (size_t i = start_index + step; i < end_index; i += step) {
    int delta_x = v_edge[i].x - v_edge[i - step].x;
    if (delta_x != 0) {
      v_slope.push_back((v_edge[i].y - v_edge[i - step].y) * 100 / delta_x);
    }
  }
  
  if (v_slope.size() > 1) {
    double sum = accumulate(begin(v_slope), end(v_slope), 0.0);
    double mean = sum / v_slope.size();
    double accum = 0.0;
    
    for_each(begin(v_slope), end(v_slope),
             [&](const double d) { accum += (d - mean) * (d - mean); });
    
    return sqrt(accum / (v_slope.size() - 1)); // 标准差
  } else {
    return 0;
  }
}
