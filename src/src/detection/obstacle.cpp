...
public:
     ...
     * @return false
     */
     void setParams4(const Motion::Params &params) {
        person = params.obstacleperson;
        cone   =   params.obstaclecone;
        discone = params.odiscone;
        disperson = params.odisperson;
    ...
    bool process(Tracking &track, vector<PredictResult> predict)
    {
        ...
        {
            if((predict[i].type == LABEL_PEDESTRIAN) && (predict[i].y + predict[i].height) > ROWSIMAGE * disperson) // AI标志距离计算
               { resultsObs.push_back(predict[i]);
               block = false;
               }
               else if((predict[i].type == LABEL_CONE) && (predict[i].y + predict[i].height) > ROWSIMAGE * discone)
               { resultsObs.push_back(predict[i]);
               block = false;
               }
               else if((predict[i].type == LABEL_BLOCK)&& (predict[i].y + predict[i].height) > ROWSIMAGE * 0.25) 
               { resultsObs.push_back(predict[i]);
               block = true;
               }
        }
        ...
        // 选取距离最近的锥桶
        // 选取距离最近的障碍物（基于底部y坐标，y + height越大表示越近）
              int maxBottomY = -1;  // 障碍物底部的y坐标（y + height）
              int index = 0;        // 目标序号
              for (size_t i = 0; i < resultsObs.size(); i++) {
                 
                     int bottomY = resultsObs[i].y + resultsObs[i].height;
        ...
        // 障碍物方向判定（左/右）
          row = track.pointsEdgeLeft.size() - (resultsObs[index].y + resultsObs[index].height - track.rowCutUp);
         cout << "row: " << row  <<endl;
        ...
        else if(resultsObs[index].type == LABEL_BLOCK)
        { 
              if (row < 61) // 无需规划路径
            {return enable;
            ...
            disLeft <= disRight) //[1] 障碍物靠左
        {
            if (resultsObs[index].type == LABEL_BLOCK)                          // 黑色路障特殊处理
            {
              ...
              /*  vector<POINT> points(4); // 三阶贝塞尔曲线
                ...
                vector<POINT> repair = Bezier(0.005, points);  // 重新规划车道线
            ...
            else if (resultsObs[index].type ==  LABEL_PEDESTRIAN)                  // 行人特殊处理
            {
                vector<POINT> points(4); // 三阶贝塞尔曲线
                points[0] = track.pointsEdgeLeft[row / 2];
                points[1] = {resultsObs[index].y + resultsObs[index].height, resultsObs[index].x + resultsObs[index].width * 3};
                points[2] = {(resultsObs[index].y + resultsObs[index].height + resultsObs[index].y) / 2, resultsObs[index].x + resultsObs[index].width * person};
                if (resultsObs[index].y > track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1].x)
                    points[3] = track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1];
                else
                    points[3] = {resultsObs[index].y, resultsObs[index].x + resultsObs[index].width * 3};
            ...
            else                                                               //锥桶
            {
                ...
                points[0] = track.pointsEdgeLeft[row / 2];
                points[1] = {resultsObs[index].y + resultsObs[index].height, resultsObs[index].x + resultsObs[index].width * 3};
                points[2] = {(resultsObs[index].y + resultsObs[index].height + resultsObs[index].y) / 2, resultsObs[index].x + resultsObs[index].width * cone};
                if (resultsObs[index].y > track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1].x)
                    points[3] = track.pointsEdgeLeft[track.pointsEdgeLeft.size() - 1];
                else
                    points[3] = {resultsObs[index].y, resultsObs[index].x + resultsObs[index].width * 3};
