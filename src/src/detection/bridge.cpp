...
{
public:
void setParams2(const Motion::Params &params) {
        
        bridgetime = params.bridgetime;
    }
    ...
    bool process(Tracking &track, vector<PredictResult> predict)
    {
        if (bridgeEnable) // 进入坡道
        {
            ...
            counterSession++;
            if (counterSession > bridgetime) // 上桥40场图像后失效
            {
                counterRec = 0;
                counterSession = 0;
                bridgeEnable = false;
        ...
        else // 检测坡道
        {
            ...
            {
                if (predict[i].type == LABEL_BRIDGE && predict[i].score > 0.6 && (predict[i].y + predict[i].height) > ROWSIMAGE * 0.4)
                {
                    counterRec++;
                    break;
                }
            ...
            {
                counterSession++;
                if (counterRec >= 2 && counterSession < 3)
                {
                    counterRec = 0;
                    counterSession = 0;
